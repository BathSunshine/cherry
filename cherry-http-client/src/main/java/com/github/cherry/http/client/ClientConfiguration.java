...
import com.github.cherry.http.client.common.utils.VersionInfoUtils;
...
public class ClientConfiguration {
    ...
    public void setProxyPort(int proxyPort) throws ClientException {
        if (proxyPort <= 0) {
            throw new ClientException(CommonResourceHolder.getInstance().getString("ParameterIsInvalid"), null);
        }
    ...
    /**
     * 返回从连接池中获取连接的超时时间（单位：毫秒）。 0表示无限等待。负值表示未定义，默认-1。
     * 
     ...
     * 
     * @param connectionRequestTimeout 设置从连接池中获取连接的超时时间。
     */
